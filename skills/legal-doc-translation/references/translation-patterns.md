...
| Пункт N.M / N.M(x) (decimal, incl. sub-items) | Clause N.M / Clause N.M(x) | RU→EN | Same word "Clause" as Статья above — RU keeps two words (Статья/Пункт), EN collapses both to one; distinguish only by decimal depth if needed | 2026-08-18 | liability/termination/expenses clauses, parallel text |
...
- **Periods/deadlines: digit + spelled-out redundancy for days/months** — confirmed by repeated real usage: «5 (пяти) Рабочих дней» → "5 (five) Business Days", «2 (двух) месяцев» → "2 (two) months", «10 (десяти) Рабочих Дней» → "10 (ten) Business Days" — matches Section F default exactly, now backed by real examples. «Рабочий день»/"Business Day" is itself a capitalized defined term.
...
- **"Исполнение" — CONFIRMED inconsistent, needs a decision.** New evidence (2026-08-18, liability/expenses clauses, same document as the Статья/Пункт finding): «неисполнение или ненадлежащее исполнение Соглашения» → "failure to **perform** or improper **performance** of the Agreement" (correct, matches ст. 15/393 ГК РФ register) — but a few clauses later, «его исполнением» (in a list: заключение/исполнение/прекращение) → "its **execution**" — same RU word, same document, opposite EN choice. This is no longer a hypothetical risk, it's an observed split. Recommend standardizing on "performance" (matches the clearer 1.1 usage and avoids the "execution = signing" collision with «заключение»/"conclusion" noted earlier) — awaiting user confirmation before treating either as the locked default.
...
- **Определённый термин, потерявший консистентность: «Заверения» → Warranty vs Representation.** Predominant rendering across the document is "Warranties" (1.2(a), 1.2(b), first occurrence in 1.5) but the same sentence in 1.5 switches to "Representation" twice for the identical RU term «Заверение(-я) Продавца». Treat "Warranties" as the intended defined term unless told otherwise — "Representation" here looks like drift, not a deliberate second term.
