...
1. **Clarify scope** if not already given: which deliverable type, the coverage period (digest) or the specific instrument/question (Alert/справка), and which practice areas matter to this client. Run this as a grilling session rather than an ad-hoc question or two: read `skills/grilling/SKILL.md` for the mechanics and its `references/playbooks.md` section A for the digest-specific tree. The ordering there matters — type, period, client and deadline are settled first, and what to include or cut is asked only after step 2 has produced a draft list of acts, so that round argues over numbered instruments instead of abstractions. Skip the session when the brief already names type, period and client; one question about the deadline is enough.
